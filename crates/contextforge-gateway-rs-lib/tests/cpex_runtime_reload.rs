...
use support::{MemoryRuntimePluginConfigStore, TestPlugin, TestPluginFactory, plugin_config, sum_request};
...
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn allow_pre_hook_preserves_absent_arguments() {
    let plugin = Arc::new(TestPlugin::new("configured-pre", vec![cmf_hook_names::TOOL_PRE_INVOKE]));
...
async fn failed_runtime_reload_keeps_current_plugin_runtime() {
    ...
    let result =
        runtime.before_tool_call(&sum_request("sum", 1, 2), "sum", "backend").await.expect("pre hook still runs");
    assert!(matches!(result.arguments, ToolArgumentsUpdate::Replace(Some(_))));
...
async fn runtime_reload_without_config_clears_current_plugin_runtime() {
    ...
    runtime.reload().await.expect("runtime reloads");
    let result = runtime.before_tool_call(&sum_request("sum", 1, 2), "sum", "backend").await.expect("pre hook skips");
